...
/// The label displayed when enablePopUpReport is set to YES
@property (strong, nonatomic) UILabel *popUpDetailLabel;
/// The label displayed when enablePopUpReport is set to YES
...
- (void)commonInit {
    ...
    _popupLabelColor = [UIColor grayColor];
    ...
    if (self.enableTouchReport == YES || self.enablePopUpReport == YES) {
        ...
        if (self.enablePopUpReport == YES && self.alwaysDisplayPopUpLabels == NO) {
            if ([self.delegate respondsToSelector:@selector(popUpViewForLineGraph:)]) {
                ...
                self.popUpDetailLabel.text = @"hello world.";
                self.popUpDetailLabel.textAlignment = 1;
                self.popUpDetailLabel.numberOfLines = 1;
                self.popUpDetailLabel.font = self.labelFont;
                self.popUpDetailLabel.textColor = [UIColor grayColor];
                self.popUpDetailLabel.backgroundColor = [UIColor clearColor];
                self.popUpDetailLabel.alpha = 0;

                self.shareButton = [[UIButton alloc] initWithFrame:CGRectMake(10, 10, 20, 20)];
                [self.shareButton setImage:[UIImage imageNamed:@"upload"] forState:UIControlStateNormal];
                self.shareButton.alpha = 0;


                self.popUpView = [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.popUpLabel.frame.size.width + 20, self.popUpLabel.frame.size.height + self.popUpDetailLabel.frame.size.height + 2)];
                ...
                [self addSubview:self.popUpDetailLabel];
...
#pragma clang diagnostic pop
            ...
            if (dotValue != BEMNullGraphValue) {
                ...
                if ([self.delegate respondsToSelector:@selector(lineGraph:imageForPointAtIndex:)]) {
                    UIImage *imageIcon = [self.delegate lineGraph:self imageForPointAtIndex:i];
                    [circleDot setImage:imageIcon];
                }
...
- (void)setUpPopUpLabelAbovePoint:(BEMCircle *)closestPoint {
    ...
    if ([self.delegate respondsToSelector:@selector(lineGraph:popUpTextForlineGraphAtIndex:)])
        self.popUpLabel.text = [self.delegate lineGraph:self popUpTextForlineGraphAtIndex:(NSInteger) closestDot.tag - DotFirstTag100];
    else if ([self.delegate respondsToSelector:@selector(popUpSuffixForlineGraph:)])
     ...
     if ([self.delegate respondsToSelector:@selector(lineGraph:popUpDetailTextForlineGraphAtIndex:)])
         self.popUpDetailLabel.text = [self.delegate lineGraph:self popUpDetailTextForlineGraphAtIndex:(NSInteger) closestDot.tag - DotFirstTag100];

     if ([self.delegate respondsToSelector:@selector(lineGraph:shouldShowActionButtonAtIndex:)])
         self.shareButton.hidden = ![self.delegate lineGraph:self shouldShowActionButtonAtIndex:(NSInteger) closestDot.tag - DotFirstTag100];




     if ([self.delegate respondsToSelector:@selector(popUpLabelActionButtonTapped:)]){
         self.shareButton.tag = closestDot.tag - DotFirstTag100;
         [self.shareButton addTarget:self.delegate action:@selector(popUpLabelActionButtonTapped:) forControlEvents:UIControlEventTouchUpInside];
     }


     if ([self.delegate respondsToSelector:@selector(shouldShowActionButtonAtIndex:)]){
         self.shareButton.tag = closestDot.tag - DotFirstTag100;
         [self.shareButton addTarget:self.delegate action:@selector(popUpLabelActionButtonTapped:) forControlEvents:UIControlEventTouchUpInside];
     }
    ...
    if (!self.usingCustomPopupView) {
        ...
        self.popUpLabel.center = CGPointMake(self.popUpView.center.x - 10, self.yCenterLabel - 5);
        self.popUpDetailLabel.center = CGPointMake(self.popUpView.center.x - 10, self.yCenterLabel + 8);
        self.shareButton.center = CGPointMake(self.popUpView.center.x + 50, self.yCenterLabel);
